...
# EAGERx IMPORTS
from eagerx_core.bridges.openai_gym.bridge import GymBridge
...
class GymObject(Object):
    ...
    def spec(spec, name: str, sensors: Optional[List[str]] = ['observation', 'reward', 'done'], actuators: Optional[List[str]] = ['action'],
        ...
        default = dict(name=name, sensors=sensors, actuators=actuators)
    ...
    def openai_gym(cls, spec, graph):
        ...
        image = EngineNode.make('GymRenderer', 'image', shape='$(default render_shape)', rate=None, process=2)

        # Create actuator engine nodes
        # Rate=None, because we will connect it to an actuator (thus uses the rate set in the agnostic specification)
        action = EngineNode.make('ActionActuator', 'action', rate=None, process=2, zero_action='$(default default_action)')
