...
class RealBridge(Bridge):
    ...
    def spec(spec, rate, process: Optional[int] = process.NEW_PROCESS, is_reactive: Optional[bool] = False, log_level: Optional[int] = ERROR):
        """
    ...
    @register.object_params(drive_launch_file=None, launch_args=[])
    def add_object(self, agnostic_params, bridge_params, node_params, state_params):
        ...
        if 'drive_launch_file' in bridge_params:
            launch_file = bridge_params['drive_launch_file']
