...
def graph_engine(idx):
    ...
    # Define engine
    engine = Engine.make("TestEngine", rate=20, sync=True, real_time_factor=1, process=process.ENVIRONMENT)

    # Initialize Environment
    env = EagerxEnv(name=f"rx_{idx}", rate=7, graph=graph, engine=engine)
