...
class ObservationSensor(EngineNode):
    ...
    def spec(spec, name: str, rate: float, process: Optional[int] = process.BRIDGE,
        ...
        params = dict(name=name, rate=rate, process=process, color=color, inputs=inputs, outputs=outputs)
    ...
    def callback(self, t_n: float, tick: Optional[Msg] = None) -> return_typehint(Float32MultiArray):
        assert isinstance(self.simulator[self.obj_name], dict), 'Simulator object "%s" is not compatible with this engine node.' % self.simulator[self.obj_name]
...
class ActionActuator(EngineNode):
    ...
    def spec(spec, name: str, rate: float, zero_action=None, process: Optional[int] = process.BRIDGE,
             inputs: Optional[List[str]] = ['tick', 'action'], outputs: Optional[List[str]] = ['action_applied'],
             color: Optional[str] = 'green'):
        """ActionActuator spec"""
        # Performs all the steps to fill-in the params with registered info about all functions.
    ...
    def initialize(self, zero_action):
        ...
        self.is_discrete = True if isinstance(self.simulator[self.obj_name]['env'].action_space,
                                              gym.spaces.Discrete) else False
        if zero_action is None:
            ...
            assert self.simulator[self.obj_name]['env'].action_space.contains(self.zero_action), f'The zero action provided for "{self.obj_name}" is not contained in the action space of this environment.'
    ...
    def callback(self, t_n: float, tick: Optional[Msg] = None, action: Optional[Float32MultiArray] = None) -> return_typehint(Float32MultiArray):
        assert isinstance(self.simulator[self.obj_name], dict), 'Simulator object "%s" is not compatible with this engine node.' % self.simulator[self.obj_name]

        # Set action in simulator for next step.
        if len(action.msgs) > 0:
            self.simulator[self.obj_name]['next_action'] = int(action.msgs[-1].data[0]) if self.is_discrete else action.msgs[-1].data
        else:
            self.simulator[self.obj_name]['next_action'] = self.zero_action
