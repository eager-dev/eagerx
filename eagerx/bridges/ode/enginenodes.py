...
class OdeOutput(EngineNode):
    ...
    def spec(spec, name: str, rate: float, process: Optional[int] = process.BRIDGE, color: Optional[str] = 'cyan'):
        ...
        params = dict(name=name, rate=rate, process=process, color=color, inputs=['tick'], outputs=['observation'])
...
class ActionApplied(EngineNode):
    ...
    def spec(spec, name: str, rate: float, process: Optional[int] = process.BRIDGE, color: Optional[str] = 'cyan'):
        ...
        # Modify default node params
        params = dict(name=name, rate=rate, process=process, color=color, inputs=['tick', 'action_applied'], outputs=['action_applied'])
        spec.set_parameters(params)
