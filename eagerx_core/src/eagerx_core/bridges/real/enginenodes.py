...
class RealRender(EngineNode):
    @staticmethod
    @register.spec('RealRender', EngineNode)
    def spec(spec, name: str, rate: float, process: Optional[int] = process.NEW_PROCESS, color: Optional[str] = 'green', shape=[480, 480], camera_idx: int = 0):
        ...
        spec.initialize(RealRender)

        # Modify default node params
        params = dict(name=name, rate=rate, process=process, color=color, inputs=['tick'], outputs=['image'])
        spec.set_parameters(params)

        # Modify custom node params
        spec.set_parameter('shape', shape)
        spec.set_parameter('camera_idx', 0)
