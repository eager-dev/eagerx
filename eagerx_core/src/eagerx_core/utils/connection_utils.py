...
def sensor_to_input(obj, obj_comp, obj_cname, node, node_comp, node_cname, converter=None, delay=None, window=None):
    assert obj_cname in obj.get_params['default'][obj_comp], '"%s" was not selected in %s of object "%s" during its initialization.' % (obj_cname, obj_comp, obj.name)
    assert node_cname in node.get_params['default'][node_comp], '"%s" was not selected in %s of node "%s" during its initialization.' % (node_cname, node_comp, node.name)

    address = '%s/%s/%s' % (obj.name, obj_comp, obj_cname)
    msg_type_A = get_cls_from_string(obj.get_params[obj_comp][obj_cname]['msg_type'])
    ...
    node.get_params[node_comp][node_cname]['address'] = address
    if converter:
        node.get_params[node_comp][node_cname]['converter'] = converter
        msg_type_C = get_opposite_msg_cls(msg_type_B, converter)
    else:
        msg_type_C = msg_type_B
    if delay:
        node.get_params[node_comp][node_cname]['delay'] = delay
    if window:
        node.get_params[node_comp][node_cname]['window'] = window

    # Verify that msg_type after converter matches the one specified in the .yaml
    msg_type_C_yaml = get_cls_from_string(node.get_params[node_comp][node_cname]['msg_type'])
...
def sensor_to_actuator(src_obj, src_obj_comp, src_obj_cname, target_obj, target_obj_comp, target_obj_cname, converter=None, delay=None, window=None):
    ...
    assert src_obj_cname in src_obj.get_params['default'][src_obj_comp], '"%s" was not selected in %s of object "%s" during its initialization.' % (src_obj_cname, src_obj_comp, src_obj.name)
    assert target_obj_cname in target_obj.get_params['default'][target_obj_comp], '"%s" was not selected in %s of target_obj "%s" during its initialization.' % (target_obj_cname, target_obj_comp, target_obj.name)

    address = '%s/%s/%s' % (src_obj.name, src_obj_comp, src_obj_cname)
    msg_type_A = get_cls_from_string(src_obj.get_params[src_obj_comp][src_obj_cname]['msg_type'])
    msg_type_B = msg_type_A  # Convert with output converter if specified (not applicable to object outputs (sensors))

    # Fill in target target_obj properties
    target_obj.get_params[target_obj_comp][target_obj_cname]['address'] = address
    if converter:
        target_obj.get_params[target_obj_comp][target_obj_cname]['converter'] = converter
        msg_type_C = get_opposite_msg_cls(msg_type_B, converter)
    else:
        msg_type_C = msg_type_B
    if delay:
        target_obj.get_params['inputs'][target_obj_cname]['delay'] = delay

    # Verify that msg_type after converter matches the one specified in the .yaml
    msg_type_C_yaml = get_cls_from_string(target_obj.get_params[target_obj_comp][target_obj_cname]['msg_type'])
...
def sensor_to_observation(obj, obj_comp, obj_cname, node, node_cname, node_comp='inputs', converter=None, delay=None, window=None):
    assert obj_cname in obj.get_params['default'][obj_comp], '"%s" was not selected in %s of object "%s" during its initialization.' % (obj_cname, obj_comp, obj.name)
    assert converter is not None or 'space_converter' in obj.get_params[obj_comp][obj_cname], '"%s" does not have a space_converter defined under %s in the .yaml of object "%s". Either specify it there, or add an input converter that acts as a space_converter to this connection entry.' % (obj_cname, obj_comp, obj.name)

    address = '%s/%s/%s' % (obj.name, obj_comp, obj_cname)
    msg_type_A = get_cls_from_string(obj.get_params[obj_comp][obj_cname]['msg_type'])
    msg_type_B = msg_type_A  # Convert with output converter if specified (not applicable to object outputs (sensors))
    if converter is None:
        converter = obj.get_params[obj_comp][obj_cname]['space_converter']
    msg_type_C = get_opposite_msg_cls(msg_type_B, converter)

    # Fill in target node properties
    node.get_params['default'][node_comp].append(node_cname)
    node.get_params[node_comp][node_cname] = dict(address=address, msg_type=get_module_type_string(msg_type_B), converter=converter)
    add_default_args(node.get_params[node_comp][node_cname], component=node_comp)
    if delay:
        node.get_params[node_comp][node_cname]['delay'] = delay
    if window:
        node.get_params[node_comp][node_cname]['window'] = window
...
def action_to_input(src_node, src_node_cname, target_node, target_node_comp, target_node_cname, src_node_comp='outputs', converter=None, delay=None, window=None):
    ...
    assert 'space_converter' in target_node.get_params[target_node_comp][target_node_cname], '"%s" does not have a space_converter defined under %s in the .yaml of object "%s".' % (target_node_cname, target_node_comp, target_node.name)
    assert target_node_cname in target_node.get_params['default'][target_node_comp], '"%s" was not selected in %s of target_node "%s" during its initialization.' % (target_node_cname, target_node_comp, target_node.name)

    address = '%s/%s/%s' % (src_node.name, src_node_comp, src_node_cname)
    space_converter = target_node.get_params[target_node_comp][target_node_cname]['space_converter']

    # Infer source properties (converter & msg_type) from target node
    msg_type_C = get_cls_from_string(target_node.get_params[target_node_comp][target_node_cname]['msg_type'])
    ...
    if src_node_cname in src_node.get_params[src_node_comp]:
        space_converter_yaml = src_node.get_params[src_node_comp][src_node_cname]['converter']
        msg_type_B_yaml = get_cls_from_string(src_node.get_params[src_node_comp][src_node_cname]['msg_type'])
        ...
        src_node.get_params['default']['outputs'].append(src_node_cname)
        src_node.get_params['outputs'][src_node_cname] = dict(msg_type=get_module_type_string(msg_type_B), converter=space_converter)
        add_default_args(src_node.get_params['outputs'][src_node_cname], component='outputs')
    ...
    # Fill in target node properties
    target_node.get_params[target_node_comp][target_node_cname]['address'] = address
    if converter:
        target_node.get_params[target_node_comp][target_node_cname]['converter'] = converter
    if delay:
        target_node.get_params[target_node_comp][target_node_cname]['delay'] = delay
    if window:
        target_node.get_params[target_node_comp][target_node_cname]['window'] = window
...
def action_to_actuator(node, node_cname, obj, obj_comp, obj_cname, node_comp='outputs', converter=None, delay=None, window=None):
    ...
    assert 'space_converter' in obj.get_params[obj_comp][obj_cname], '"%s" does not have a space_converter defined under %s in the .yaml of object "%s".' % (obj_cname, obj_comp, obj.name)
    assert obj_cname in obj.get_params['default'][obj_comp], '"%s" was not selected in %s of object "%s" during its initialization.' % (obj_cname, obj_comp, obj.name)

    address = '%s/%s/%s' % (node.name, node_comp, node_cname)
    space_converter = obj.get_params[obj_comp][obj_cname]['space_converter']

    # Infer source properties (converter & msg_type) from target node
    msg_type_C = get_cls_from_string(obj.get_params[obj_comp][obj_cname]['msg_type'])
    if converter:  # Overwrite msg_type_B if input converter specified
        msg_type_B = get_opposite_msg_cls(msg_type_C, converter)
    else:
        msg_type_B = msg_type_C
    ...
    if node_cname in node.get_params[node_comp]:
        space_converter_yaml = node.get_params[node_comp][node_cname]['converter']
        msg_type_B_yaml = get_cls_from_string(node.get_params[node_comp][node_cname]['msg_type'])
        ...
        node.get_params['default']['outputs'].append(node_cname)
        node.get_params['outputs'][node_cname] = dict(msg_type=get_module_type_string(msg_type_B), converter=space_converter)
        add_default_args(node.get_params['outputs'][node_cname], component='outputs')
        msg_type_A = get_opposite_msg_cls(msg_type_C, space_converter)
        if converter:
            assert msg_type_B == msg_type_C, 'Cannot have a converter that maps to a different msg_type when also using the space_converter.'

    # Fill in target node properties
    obj.get_params[obj_comp][obj_cname]['address'] = address
    if converter:
        obj.get_params[obj_comp][obj_cname]['converter'] = converter
    if delay:
        obj.get_params[obj_comp][obj_cname]['delay'] = delay
    if window:
        obj.get_params[obj_comp][obj_cname]['window'] = window
...
def output_to_input(src_node, src_node_cname, target_node, target_node_comp, target_node_cname, src_node_comp='outputs', converter=None, delay=None, window=None):
    assert target_node_cname in target_node.get_params['default'][target_node_comp], '"%s" was not selected in %s of target_node "%s" during its initialization.' % (target_node_cname, target_node_comp, target_node.name)
    assert src_node_cname in src_node.get_params['default'][src_node_comp], '"%s" was not selected in %s of src_node "%s" during its initialization.' % (src_node_cname, src_node_comp, src_node.name)

    address = '%s/%s/%s' % (src_node.name, src_node_comp, src_node_cname)
    msg_type_A = get_cls_from_string(src_node.get_params[src_node_comp][src_node_cname]['msg_type'])
    msg_type_B = get_opposite_msg_cls(msg_type_A, src_node.get_params[src_node_comp][src_node_cname]['converter'])
    ...
    # Verify that msg_type after converter matches the one specified in the .yaml
    msg_type_C_yaml = get_cls_from_string(target_node.get_params[target_node_comp][target_node_cname]['msg_type'])
...
def output_to_actuator(node, node_cname, obj, obj_comp, obj_cname, node_comp='outputs', converter=None, delay=None, window=None):
    ...
    address = '%s/%s/%s' % (node.name, node_comp, node_cname)
    msg_type_A = get_cls_from_string(node.get_params[node_comp][node_cname]['msg_type'])
    msg_type_B = get_opposite_msg_cls(msg_type_A, node.get_params[node_comp][node_cname]['converter'])
    ...
    # Verify that msg_type after converter matches the one specified in the .yaml
    msg_type_C_yaml = get_cls_from_string(obj.get_params[obj_comp][obj_cname]['msg_type'])
...
def output_to_feedthrough(src_node, src_node_cname, target_node, target_node_comp, target_node_cname, src_node_comp='outputs', converter=None, delay=None, window=None):
    assert target_node_cname in target_node.get_params['default']['outputs'], '"%s" was not selected in %s of target_node "%s" during its initialization.' % (target_node_cname, 'outputs', target_node.name)
    assert src_node_cname in src_node.get_params['default'][src_node_comp], '"%s" was not selected in %s of src_node "%s" during its initialization.' % (src_node_cname, src_node_comp, src_node.name)
    ...
    # Verify that msg_type after converter matches the one specified in the .yaml
    msg_type_C_yaml = get_cls_from_string(target_node.get_params['outputs'][target_node_cname]['msg_type'])
    assert msg_type_C == msg_type_C_yaml, 'Msg_type "%s" does not match msg_type "%s" specified in the .yaml of target_node "%s".' % (msg_type_C, msg_type_C_yaml, target_node.name)
    return msg_type_A, msg_type_B, msg_type_C
...
def output_to_observation(src_node, src_node_cname, target_node, target_node_cname, target_node_comp='inputs', src_node_comp='outputs', converter=None, delay=None, window=None):
    assert src_node_cname in src_node.get_params['default'][src_node_comp], '"%s" was not selected in %s of src_node "%s" during its initialization.' % (src_node_cname, src_node_comp, src_node.name)
    assert converter is not None or 'space_converter' in src_node.get_params[src_node_comp][src_node_cname], '"%s" does not have a space_converter defined under %s in the .yaml of object "%s".. Either specify it there, or add an input converter that acts as a space_converter to this connection entry.' % (src_node_cname, src_node_comp, src_node.name)

    address = '%s/%s/%s' % (src_node.name, src_node_comp, src_node_cname)
    msg_type_A = get_cls_from_string(src_node.get_params[src_node_comp][src_node_cname]['msg_type'])
    msg_type_B = get_opposite_msg_cls(msg_type_A, src_node.get_params[src_node_comp][src_node_cname]['converter'])
    if converter is None:
        converter = src_node.get_params[src_node_comp][src_node_cname]['space_converter']
    msg_type_C = get_opposite_msg_cls(msg_type_B, converter)

    # Fill in target node properties
    target_node.get_params['default'][target_node_comp].append(target_node_cname)
    target_node.get_params[target_node_comp][target_node_cname] = dict(address=address, msg_type=get_module_type_string(msg_type_B), converter=converter)
    add_default_args(target_node.get_params[target_node_comp][target_node_cname], component=target_node_comp)
    if delay:
        target_node.get_params[target_node_comp][target_node_cname]['delay'] = delay
    if window:
        target_node.get_params[target_node_comp][target_node_cname]['window'] = window
